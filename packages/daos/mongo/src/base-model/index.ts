...
export default class MongoBase {
  ...
  static async getList<T>(query: Filter<T> = {}, limit: number = 10, skip: number = 0, sortBy: string = null, asc: boolean = true): Promise<T[]> {
    const options: any = {};
    options.limit = limit;
    if (sortBy) {
      options.sort = {
  ...
  static async getCounter<T>(query: Filter<T> = {}): Promise<number> {
    return MongoBase.dao.count(this.collectionName, query, {});
  }

  async save() {
    this.created = new Date();
    const response: any = await MongoBase.dao.insertOne(this.constructor.collectionName, this);
  ...
  async update() {
    if (this._id) {
      ...
      const response: any = await MongoBase.dao.updateOne(this.constructor.collectionName, { _id: this._id }, { $set: fields });
      return response?.result?.ok ?? false;
    }
    return false;
  }
  ...
  async patch(fields: any): Promise<boolean> {
    if (this._id) {
      this.lastEdit = new Date();
      const response: any = await MongoBase.dao.updateOne(this.constructor.collectionName, { _id: this._id }, {
        $set: fields,
      });
      if (response?.result?.ok) {
  ...
  async remove(): Promise<boolean> {
    if (this._id) {
      const response: any = await MongoBase.dao.deleteOne(this.constructor.collectionName, {
        _id: this._id,
      });
